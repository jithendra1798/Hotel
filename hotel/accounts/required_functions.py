...
def validate(request,user_password, user_password1, user_aadhaar, user_email, user_phone):
    ...
    if validate_email(user_email):
        if Hotel_User.objects.filter(user_email=user_email).exists() or User.objects.filter(email=user_email).exists():
    ...
    if len(str(user_aadhaar))==12:
        if Hotel_User.objects.filter(user_aadhaar=user_aadhaar).exists():
